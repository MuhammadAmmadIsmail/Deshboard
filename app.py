...
# ========================================== 
# INSTALL CATBOOST DYNAMICALLY
# ========================================== 
@st.cache_resource
def install_catboost():
    """Install CatBoost on first run (takes ~2 min)"""
    try:
        import catboost
        return True
    except ImportError:
        with st.spinner("⏳ Installing CatBoost (first run only, ~2 min)..."):
            try:
                subprocess.check_call([sys.executable, "-m", "pip", "install", "--no-cache-dir", "catboost==1.2.2", "-q"])
                st.success("✅ CatBoost installed! Reloading app...")
                st.rerun()
            except Exception as e:
                st.error(f"❌ Failed to install CatBoost: {e}")
                return False
        return True

# Install CatBoost if needed
catboost_ready = install_catboost()
...
if catboost_ready:
    ...
    model_event, model_magnitude, model_traffic, train_medians, optimal_threshold = load_models()
    df = load_data()
    data_loaded = (model_event is not None and df is not None)
    ...
    if data_loaded:
        ...
        y_event_pred_base = (y_event_prob >= optimal_threshold).astype(int)
        if y_event_pred_base.sum() > 0:
            ...
            y_magnitude_pred[y_event_pred_base == 1] = model_magnitude.predict(event_pool)
    ...
    st.error("❌ Failed to install CatBoost. Please check dependencies.")